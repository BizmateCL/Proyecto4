...
const crearReservas = (req, res) => {
  ...
  for (const reserva of nuevasReservas) {
    const { hotel, fecha, tipoHabitacion, adultos, niños, estado_pago } = reserva;
    ...
    // Crear la reserva con un ID único y estado de pago predeterminado
    reservasValidas.push({
      ...
      estado_pago: estado_pago || "pendiente", // Valor predeterminado si no se envía
    });
  }
...
const actualizarReservaPorId = (req, res) => {
  ...
  const { hotel, fecha, tipoHabitacion, adultos, niños, estado_pago } = req.body; // Datos enviados en el cuerpo de la solicitud
  ...
  reservas[indiceReserva] = {
    ...
    estado_pago: estado_pago || reservas[indiceReserva].estado_pago, // Actualizar estado_pago si se envía
  };
